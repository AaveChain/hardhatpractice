...
contract SupplyChain{
    ...
    event IssuesCerticate(address indexed issuer,address indexed prover, uint certificateId);
    ...
    function unmarshalMode(string memory _mode) private pure returns(Mode mode){
        ...
        if(encodedMode == encodedMode0){
            return Mode.ISSUER;
        }else if(encodedMode0 == encodedMode1){
            return Mode.PROVER;
        }else if(encodedMode == encodedMode2){
            return Mode.VERIFIER;
        }

        revert("recieved invalid entity node");
    }

    function AddVaccinBatch(string memory brand, address manufacturer) public returns(uint){
    ...
    function issuesCertificate(address _issuer, address _prover, string memory _status, uint vaccineBatchId, bytes memory signature) public returns(uint){
        ...
        emit IssuesCerticate(_issuer, _prover, certificateIds.length-1);
